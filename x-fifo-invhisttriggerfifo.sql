...
DECLARE
	...
	invbuy_curs CURSOR (c_itemsite_id integer, c_ordnumber text) FOR
		...
		FROM
			...
			invhist AS i
		WHERE a2.invhist_id = i.invhist_id;
        
			
	invsell_curs CURSOR (c_itemsite_id integer, c_ordnumber text) FOR
		SELECT a2.ordernumber AS ordernumber, a2.itemsite_id AS itemsite_id, 
			   a2.qty AS qty, a2.current_totalcost AS current_totalcost, a2.current_unitcost AS current_unitcost,
			   i.invhist_id AS invhist_id, i.invhist_transdate AS transdate
		FROM
			(
				SELECT
...
BEGIN
    ...
    IF NEW.invhist_transtype = 'RP' 
        ...
        (NEW.invhist_transtype = 'AD' AND v_trans_qty > 0) THEN
            ...
            OPEN invbuy_curs(NEW.invhist_itemsite_id, NEW.invhist_ordnumber);
            
            FETCH invbuy_curs INTO new_inv_buy;
...
-- WHAT happens when this query fails?   
    ...
    ELSIF
        ...
        NEW.invhist_transtype = 'RS' THEN
            ...
            OPEN invsell_curs(NEW.invhist_itemsite_id, NEW.invhist_ordnumber);
            
            FETCH invsell_curs INTO new_inv_sell;
